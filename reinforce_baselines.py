...
class RolloutBaseline(Baseline):
    ...
    def _update_model(self, model, epoch, dataset=None):
        ...
        if dataset is None:
            if self.opts.problem == 'MultiPM':
                self.dataset = self.problem.make_dataset(
                    size=self.opts.graph_size, num_samples=self.opts.val_size, pk=self.opts.pk,
                    r=self.opts.r, distribution=self.opts.data_distribution)
            else:
