...
class Decoder(nn.Module):
    ...
    def forward(self, decoder_input, embedded_inputs, hidden, context, eval_tours=None):
        ...
        #     requires_grad=False
        # )
        mask = torch.zeros(batch_size, n, dtype=torch.bool, device=context.device)
...
class PointerNetwork(nn.Module):
    ...
    def forward(self, inputs, eval_tours=None, return_pi=False):
        loc = inputs['loc']
        batch_size, graph_size, input_dim = loc.size()

        embedded_inputs = torch.mm(
            loc.transpose(0, 1).contiguous().view(-1, input_dim),
            self.embedding
        ...
        if self.is_PM or self.is_PC:
            cost = self.problem.get_total_dis(inputs, pi)
        elif self.is_MCLP:
            cover_num = self.problem.get_total_num(inputs, pi)
            cost = -cover_num
        elif self.is_LSCP:
            cost = self.problem.get_facility_num(inputs, pi)
        else:
